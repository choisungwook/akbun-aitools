...
akbun 발표자료 두 모드의 실행본이다. 상수와 헬퍼를 그대로 복사해 시작한다(`npm i pptxgenjs`).
두 키트 모두 실제 렌더링으로 검증된 코드다.
...
확인 항목: 텍스트 잘림·겹침(라벨과 마커, 라벨과 화살표), 모드별 색 문법 위반, 헤더 위치·형식
통일, 페이지 번호(외부 발표), 슬라이드당 메시지 한 줄(스터디).
