...
- **원문자 번호**: ①②③ 스타일. 원 안에 숫자.
- **소품은 크게**: 도구·물건 같은 소품은 옆 캐릭터 키의 60~100% 크기로 그린다. 기본 도형 3~6개로
  단순하게 만들되, 실루엣만으로 무엇인지 읽혀야 한다. 칸이 좁으면 캐릭터를 줄이고 소품을 키운다.
...
hand lettering, big readable props, generous even margins — nothing touches the border
or the band lines.

DO NOT: use any color, add gradients or textures, use any animal other than the whale
and the rabbit, draw props tiny or ambiguous, misspell any text, or let elements overlap
...
- Left: ribbon label reading exactly: "BREWS IN PRACTICE", with 3 mini-columns, each a
  ...
  "Camp Brew" (a whale pouring over a campfire), "Cafe >300 cups/day" (a whale barista
  handing a cup across the counter to a rabbit customer).
