...
프롬프트와 SVG 모두 아래 수치를 그대로 따른다. 좌표는 1000×700 기준 비율이다.

- **비율**: 가로형 10:7 (SVG viewBox `0 0 1000 700`).
- **테두리**: 가장자리에서 5 안쪽, 굵기 3의 검은 프레임. 모든 요소 위에 얹는다.
- **제목**: 가로 중앙 정렬, 베이스라인 y=86 (높이의 약 12%). 자간을 글자 크기의 절반 이상으로 넓게.
- **지평선**: y=301 (높이의 43%). 위는 밝은 회색 하늘/배경, 아래는 중간 회색 바닥.
- **바닥**: 아래로 갈수록 조금 어두워진다. 짧은 흰색 대시 마크 6~9개를 흩뿌린다.
- **주 피사체**: 중앙에서 약간 왼쪽(x≈370), 위 y≈180에서 바닥 y≈610까지 크게 세운다.
- **보조 피사체** (선택): 오른쪽 중경(x≈720), 주 피사체의 60% 이하 크기.
- **큰 오브젝트** (선택): 탈것·건물 등은 지평선에 걸쳐 피사체 뒤에 낮게 눕힌다.
- **캡션**: 가로 중앙 정렬, 베이스라인 y=648 (높이의 약 92%).
- **여백**: 제목 위, 캡션 아래, 좌우 모두 이 좌표에서 자연스럽게 생기는 여백을 유지한다. 요소를 테두리에 붙이지 않는다.
...
```text
A minimal flat vector poster card, landscape 10:7 aspect ratio, framed by a thin black
rectangular border just inside the card edge. Muted monochrome palette: off-white, light gray,
mid gray, near-black — plus exactly ONE accent color, <ACCENT COLOR, e.g. "burnt orange">,
used only on small areas.

TOP (inside the frame, upper 12% of the card): a movie-title-style heading in an elegant serif
...
- Main subject, large, standing slightly left of center: <MAIN SUBJECT — one sentence,
  e.g. "a diver in a bulky suit facing the viewer, helmet visor in the accent color">.
- Secondary subject, smaller, mid-ground right: <SECONDARY SUBJECT>. (delete if none)
- Behind them, lying low across the horizon: <LARGE OBJECT, e.g. "a sleek dark submarine
  resting on the ground">. (delete if none)

BOTTOM (inside the frame, lower 8% of the card): a single handwritten-style caption line,
...
- `viewBox="0 0 1000 700"`, 외부 이미지·스크립트 없는 순수 SVG 1.1로 작성한다. Figma와 Canva가 그대로 불러온다.
...
A minimal flat vector poster card, landscape 10:7 aspect ratio, framed by a thin black
rectangular border just inside the card edge. Muted monochrome palette: off-white, light gray,
mid gray, near-black — plus exactly ONE accent color, burnt orange, used only on small areas.
...
- 레이아웃 수치(제목 12%, 지평선 43%, 캡션 92%, 테두리)를 프롬프트와 SVG 양쪽에 지켰는가?
