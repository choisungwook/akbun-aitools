...
2. Read only the supporting resources involved in the change.
3. Keep invocation scope and runtime behavior in `SKILL.md`; keep stable maintenance context and terminology in this wiki.
4. When adding or removing a check, update the six-check table in `SKILL.md`, the output template, and the responsibility line in `architecture.md` together.
...
```bash
uv run --python 3.12 --with pyyaml python "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/quick_validate.py" plugins/akbun-writing/skills/akbun-thumbnail-review
```

Check that every check in `SKILL.md` has a 방법 and 이유 paragraph, that the output template lists the same six checks, and that both plugin manifests use the release version required by the root AGENTS.md.
