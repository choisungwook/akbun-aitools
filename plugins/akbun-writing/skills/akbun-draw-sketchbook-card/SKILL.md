...
- **비어 있는 요소는 굳이 만들지 않는다.** 시리즈 번호가 없으면 번호 없이, 부제가 없으면 부제 없이 카드를 만든다. 페이지 배지(`7/12` 같은 표시)는 사용자가 시리즈 정보를 줬을 때만 넣는다.
...
- **제목**: 대문자 손글씨 영어 제목 (`ORBIT`)
- **부제** (선택): 제목 아래 괄호 안 작은 손글씨. 한국어 가능 (`(아크샷)`)
- **밑줄**: 제목 블록 아래 짧고 굵은 손그림 밑줄
- **설명**: 한두 줄 손글씨 설명 (`피사체를 중심으로 원을 그리며 이동`)
- **체크리스트**: 손그림 체크마크로 시작하는 항목 2~4개 (`영웅 연출`, `제품 광고`, `시네마틱 무드`)
- **일러스트**: 오른쪽 약 2/3 영역, 얇은 연필 테두리 액자 안의 정밀한 연필 그림 (황량한 능선에 선 우주비행사)
- **주석 오버레이** (선택이지만 강력 추천): 일러스트 위에 굵은 연필 선으로 덧그린 개념 표시 (피사체를 도는 점선 타원 화살표)
- **페이지 배지** (선택): 오른쪽 위 모서리 어두운 둥근 배지 안 밝은 글씨 (`7/12`)

## 장면과 주석 고르기 (이 skill의 핵심)

카드의 힘은 텍스트가 아니라 **장면 하나 + 주석 하나**가 개념을 즉시 이해시키는 데서 나온다.

- **장면은 피사체가 하나뿐인 단순한 장면**으로 고른다. 레퍼런스는 "황량한 풍경 능선 위에 홀로 선 우주비행사" 하나로 orbit shot의 대상(영웅)을 만들었다. 배경은 넓고, 주인공은 한 명(하나)이다.
...
4. **선택 요소 정리.** 번호·부제·페이지 배지 중 근거 없는 것은 뺀다.
...
PAGE BADGE: a small dark rounded rectangle in the top-right corner of the image, with light
handwriting inside reading exactly: "<N/M>". (delete if none)
...
이 예시는 레퍼런스 카드를 그대로 재현하는 완성 프롬프트다. 새 프롬프트의 품질 기준으로 삼는다.

입력 예:

```text
카메라 무빙 시리즈 7번째. ORBIT (아크샷). 피사체를 중심으로 원을 그리며 이동.
용도: 영웅 연출, 제품 광고, 시네마틱 무드. 시리즈는 총 12장.
```

skill이 한 판단: 카드 텍스트는 사용자가 준 그대로 썼다. 장면은 "orbit shot의 대상이 될 영웅" 하나가 필요해, 황량한 능선 위에 홀로 선 우주비행사의 뒷모습을 골랐다. 주석은 개념 정의("원을 그리며 이동")를 그대로 보여주는, 피사체를 수평으로 도는 점선 타원 화살표다. 시리즈 정보가 있어 페이지 배지 `7/12`를 넣었다.
...
LEFT COLUMN (about one third of the page width, hand-lettered in pencil, top to bottom):
- A large handwritten index number, reading exactly: "07".
- A large hand-lettered title in capital letters, reading exactly: "ORBIT".
...
- A short handwritten description over two lines, in Korean, reading exactly:
  "피사체를 중심으로 원을 그리며 이동".
- A vertical checklist; each item starts with a bold hand-drawn checkmark:
  - reading exactly: "영웅 연출"
  - reading exactly: "제품 광고"
  - reading exactly: "시네마틱 무드"

RIGHT SIDE (about two thirds of the page): a thin hand-drawn pencil rectangle frames a detailed
graphite pencil illustration: a lone astronaut in a spacesuit seen from behind, standing on a
rocky ridge in the foreground, overlooking a vast desolate mountain landscape under a hazy sky.
Fine hatching, rich pencil shading, strong sense of depth.

ANNOTATION OVERLAY: drawn on top of the illustration in bolder pencil strokes than the drawing
itself: a large dashed ellipse circling horizontally around the astronaut at waist height,
with an arrowhead at one end showing the direction of the camera's circular movement. It reads
like a concept diagram sketched over the illustration.

PAGE BADGE: a small dark rounded rectangle in the top-right corner of the image, with light
handwriting inside reading exactly: "7/12".
...
한국어 한 줄 설명: 사용자가 준 카드 텍스트(07 / ORBIT / 아크샷 / 설명 / 체크 3개)를 그대로 넣고, orbit shot의 대상으로 능선 위 우주비행사 뒷모습 장면을 골라 피사체를 도는 점선 타원 화살표로 "원을 그리며 이동"을 표현했습니다.
...
- 번호·부제·페이지 배지 등 근거 없는 선택 요소를 지웠는가?
