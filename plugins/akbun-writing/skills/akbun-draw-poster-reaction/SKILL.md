---
name: akbun-draw-poster-reaction
description: >
  상황·주제·이미지를 "왼쪽 인물이 갸웃하며 오른쪽 모니터 속 대상의 말풍선을 바라보는" 한 컷 손그림
  리액션 포스터로 만든다. 산출물은 두 가지다: 이미지 생성 모델(GPT image, nano-banana 등)에 그대로 넣을
  영어 프롬프트와, Figma/Canva로 가져와 편집할 수 있는 SVG 파일. 말풍선 문구를 사용자가 주면 그대로
  쓰고, 없으면 내용을 분석해 skill이 만든다.
  Trigger on: "리액션 포스터", "한 컷 포스터", "반응 포스터", "모니터 포스터", "말풍선 포스터 프롬프트",
  "reaction poster", "one-panel reaction image", "포스터 SVG", "figma svg 포스터", "canva svg 포스터",
  or any request to turn a situation into a one-panel reaction poster prompt or SVG.
---

# 한 컷 리액션 포스터 프롬프트 + SVG 생성

## 이 skill이 하는 일

하나의 상황을 **한 컷 리액션 포스터**로 보여주는 결과물을 만든다. 장면은 고정이다: 왼쪽에서 인물이
턱을 괴고 갸웃(오렌지 물음표)하며, 오른쪽 책상 위 모니터 속 대상이 말풍선으로 한마디 하는 장면이다.
"화면 속 대상이 하는 말"과 "그걸 보는 사람의 갸웃"의 대비로 상황을 즉시 전달한다. AI의 이상한 답변,
예상 밖의 알림, 어이없는 결과 화면처럼 **화면이 사람에게 말을 거는 상황**에 어울린다.

이 스타일과 레이아웃은 참고 사진 한 장을 분석해 뽑아낸 것이다. **그 사진을 그대로 재현(모창)하는
게 목적이 아니라, 사진에서 읽어낸 인물 배치·간격·톤을 새로운 상황에 맞는 새 장면으로 그리는 것**이
목적이다. 매번 화면 속 대상과 말풍선 문구는 입력 상황에 맞게 새로 정하고, 고정되는 건 레이아웃과
선·색 스타일뿐이다.
...
2. **SVG 파일** — 작업 디렉터리에 `<주제-slug>-poster.svg`로 저장하고 경로를 알려준다.
...
- **참고 이미지를 줬으면 내용을 분석한다.** 이미지에서 상황·대상·감정을 읽어 화면 속 대상과 말풍선
  문구 후보를 정한다. 참고 이미지는 그대로 복제할 픽셀 대상이 아니라, 상황을 파악하기 위한 참고
  자료다. 장면 내용(화면 속 대상, 말풍선 문구)은 입력마다 새로 만들고, 레이아웃과 스타일만 아래
  고정 규칙을 따른다.
...
## 비주얼 스타일 (레퍼런스에서 추출, 고정)

모든 산출물은 아래 스타일을 그대로 따른다. 스타일은 고정하지만, 화면 속 대상과 말풍선 문구 같은
장면 내용은 매번 새로 만든다.

- **매체**: 균일한 굵기의 진회색 잉크 라인으로 그린 손그림 포스터. 면은 플랫한 회색 톤으로만 칠한다.
...
아래 영어 템플릿의 `<...>`를 채워 완성한다. `<ON-SCREEN SUBJECT>`, `<SPEECH LINE>` 등 장면 내용은
입력 상황에 맞게 새로 정하고, 그 외 레이아웃·팔레트·스타일 문구는 바꾸지 않는다.

```text
A single-panel hand-drawn editorial illustration poster, landscape 7:5 aspect ratio, drawn with
uniform dark-gray ink outlines (#3A3A3A) and flat gray shading on a warm off-white paper
background (#F4F2ED), like a friendly illustration in a tech book.
...
이 예시는 위 스타일을 적용한 완성 산출물이다. 새 산출물의 품질 기준으로 삼는다. 참고용 예시일 뿐,
다른 상황을 받으면 화면 속 대상과 말풍선 문구는 그 상황에 맞게 다시 정한다.
...
- 화면 속 대상과 말풍선 문구를 레퍼런스 그대로 베끼지 않고 입력 상황에 맞게 새로 정했는가?
  (레퍼런스는 레이아웃·스타일 참고용, 장면 내용의 복제 대상이 아니다)
