...
- onboarding doc(온보딩 문서): 맥락이 전혀 없는 사람이 첫날 한 번 읽는 IT 시스템 소개 문서다. 코드를 읽으면 언젠가 아는 것(구조·API 목록)은 빼고, 코드에 없는 것(고객 가치, 도메인 용어와 코드 식별자의 연결, 왜 이렇게 생겼는지)에 분량을 몰아준다. 1부는 비개발자도 읽는 시스템 맥락, 2부는 개발자용 코드 진입으로 나눈다. 구조 시각화는 `akbun-analysiscode` 산출물을 재사용하고 다시 분석하지 않는다. skill: `akbun-it-onboarding`.
