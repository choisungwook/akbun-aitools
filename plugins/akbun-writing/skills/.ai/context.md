...
- onboarding doc(온보딩 문서): 직무와 관계없이 서비스를 처음 접하는 사람이 전체 모습을 이해하기 위한 IT 시스템 소개 문서다. 사용자가 제공한 코드·문서에서 고객 가치, 업무 흐름, 외부 의존성, 인프라, 배포를 확인한다. 연관 부서, 업무 규칙의 예외, 알려진 제약, 현재 방식의 이유와 기타 특이사항은 사용자 인터뷰로 보완하며 추정하지 않는다. 코드 진입·로컬 실행은 다루지 않고, 구조 시각화는 `akbun-analysiscode` 산출물을 재사용한다. skill: `akbun-it-onboarding`.
