...
- **연결은 방향 화살표**로 그린다. 트래픽이 흐르는 방향을 화살표 머리로 표시한다. 양방향이면 양쪽 화살표. **모든 화살표는 검정색으로 통일한다.** 강조는 화살표 색이 아니라 박스/글씨 색으로 한다(아래 참고).
- **연결 위 주석**(프로토콜·identity·포트·암호화처럼 그 연결을 설명하는 짧은 정보)은 **주황색 글씨**로 화살표 옆에 단다. 예: `mTLS`, `HTTP/2`, `gRPC`, `:443`, `spiffe://trust/svcA`. 첨부 그림에 주황 주석이 있으면 그 위치와 내용을 그대로 따른다.
- 주석은 그 연결을 이해하는 데 필요한 것만 단다. 모든 화살표에 라벨을 달면 시끄러워지므로, 의미 있는 연결에만 붙인다.

## 중요한 애플리케이션→네트워크 연결 강조 (초록색)

그림에서 독자가 꼭 봐야 할 핵심 경로 — 보통 **애플리케이션이 네트워크 컴퍼넌트로 트래픽을 넘기는 첫 구간**(예: 앱의 트래픽이 sidecar/CNI에 캡처되는 지점, client가 gateway로 진입하는 지점) — 은 **초록색**으로 강조한다. 화살표 색은 검정으로 두고, 그 구간을 **초록 네모박스로 감싸거나** 출발/도착 박스 또는 라벨을 **초록 글씨**로 표시한다(화살표 자체는 초록으로 칠하지 않는다).
...
입력 예: Istio ambient mesh 흐름 그림 한 장. 왼쪽 클러스터(App A/C/B, CNI, ztunnel), 가운데 App D waypoint proxy, 오른쪽 클러스터(App B/D/C, CNI, ztunnel). 화살표에 mTLS와 spiffe identity 주석. App A → CNI → ztunnel 캡처 경로는 초록.

skill이 한 분류(자동): App A/B/C/D는 애플리케이션(네모박스). CNI, ztunnel, App D waypoint proxy는 트래픽을 중계하므로 네트워크 컴퍼넌트(주황 박스). 두 클러스터를 그룹 박스로 감쌌다. 핵심 경로(App A의 트래픽이 CNI를 거쳐 ztunnel에 캡처되는 구간)를 초록으로 강조했다.
...
한국어 한 줄 설명: App들은 네모박스로, 트래픽을 중계하는 CNI·ztunnel·waypoint proxy는 주황 박스로 구분하고, mTLS·spiffe identity 주석은 주황 글씨로, App A가 CNI를 거쳐 ztunnel에 캡처되는 핵심 경로는 초록 박스로 강조했습니다. 화살표는 모두 검정입니다.
