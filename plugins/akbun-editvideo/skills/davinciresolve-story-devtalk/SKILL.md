...
`<출력 폴더>/edit-log_<YYYYMMDD_HHMM>.md`에 아래 구조로 쓴다. 각 단계 skill이 자기 절을 채운다. 9단계에서 읽는 여행 skill들이 쓰는 절 이름은 여기서 바꿔 읽는다. `4. 노출·화이트밸런스`·`5. LUT`·`6. 모자이크`는 `색` 절 아래 소제목으로, `8. 오디오`는 `오디오` 절로 쓴다.
