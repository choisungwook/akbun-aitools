...
description: >
  Electron.js 데스크톱 앱의 앱 아이콘(설치 아이콘·dock·작업표시줄 아이콘)을 귀여운 마스코트
  캐릭터로 만드는 skill. 사용자와 짧은 인터뷰로 앱의 핵심 기능을 확인한 뒤, 그 기능을
  상징하는 소품을 든 캐릭터 아이콘의 이미지 생성 프롬프트를 만들고, 생성된 이미지를 KB 수준으로
...
Electron.js 앱의 기본 아이콘을 대체할 **마스코트 캐릭터 아이콘 1개**를 만든다.
...
캐릭터 1개 + 앱 기능을 상징하는 소품 1개 + 단색 배경. 텍스트는 넣지 않는다.

## 작업 순서

1. **인터뷰.** 아래 인터뷰 질문으로 앱의 핵심 기능을 파악한다.
2. **캐릭터·소품·포즈 결정.** 캐릭터 하나, 기능을 상징하는 소품 하나와 포즈를 정해 사용자에게
   한 줄로 보여준다.
...
4. **아이콘에 쓸 캐릭터가 있는가?** 없으면 skill이 앱 성격에 맞는 단순한 마스코트를 하나 제안한다.

## 아이콘 스타일 스펙 (고정)

- **비율 1:1, 1024×1024로 생성.** Electron/electron-builder가 요구하는 원본 크기다.
- **캐릭터는 단순하게.** 둥근 몸통, 점 눈 2개, 곡선 입, 두껍고 둥근 손그림 외곽선, 플랫 컬러.
  16px에서도 실루엣이 읽히도록 디테일을 넣지 않는다.
- **소품은 크게.** 기능을 상징하는 소품 1개를 캐릭터 키의 60~100% 크기로 그린다.
  소품 실루엣만으로 기능이 읽혀야 한다.
- **배경은 단색 1가지.** 그라디언트·패턴·질감·그림자를 넣지 않는다. 플랫 컬러만 쓴다
  (이게 파일 크기를 KB 수준으로 만드는 핵심이기도 하다).
- **여백.** 캐릭터+소품을 캔버스 중앙에 두고, 가장자리에서 캔버스의 약 12%를 비운다.
...
CHARACTER: <a cute kawaii mascot, e.g. "a round cat mascot with a cream body, two small
ears, two black dot eyes and a small curved smile">. Thick rounded hand-drawn black
outlines, simple geometric shapes, flat colors only.

SCENE: the character is centered, <POSE — e.g. holding / sitting on / pointing at>
a big <PROP that symbolizes the app's function>, drawn as large as the character itself,
recognizable by silhouette alone. Only the character and this one prop — nothing else.
...
skill이 한 판단: 대표 기능은 "노트 작성"이므로 소품을 큰 연필 한 자루로 정하고, 캐릭터는
둥근 고양이 마스코트로, 차분한 앱 성격에 맞춰 배경을 크림색(#FFF6E3)으로 제안했다.
...
CHARACTER: a cute kawaii cat mascot: chubby rounded cream body, two small round ears,
a short tail, two black dot eyes, a small curved smile. Thick rounded hand-drawn black
outlines, simple geometric shapes, flat colors only.

SCENE: the character is centered, hugging a big yellow pencil as large as the character
itself, recognizable by silhouette alone. Only the character and this one prop — nothing else.
...
- 캐릭터가 단순하고 16px에서 실루엣으로 읽히는가? 소품이 캐릭터 키의 60% 이상인가?
