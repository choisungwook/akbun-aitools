...
description: >
  ...
  따뜻한 크림색 종이 배경 + 굵은 손글씨 제목 + 화살표(→)로 인과를 잇는 손글씨
  ...
  "손글씨 카드뉴스", "설명 카드뉴스 그려", "개념 카드뉴스", "cream card news",
...
카드 한 장에서 뽑아낸 것이다: 따뜻한 크림색 종이 배경, 왼쪽 위에 크게 쓴 굵은 손글씨 제목,
화살표(`→`)로 원인과 결과를 잇는 손글씨 본문 두어 줄, 그리고 카드
...
- **제목**: 왼쪽 위에 **굵은 마커 손글씨** 한 줄. 검정. 카드에서 가장 큰 글씨다.
...
이 스타일은 기본적으로 캐릭터 없이 도형과 글씨만으로 설명한다. 소재상 인물이 꼭 필요하면 소재에
맞는 캐릭터를 하나 정해 같은 검정 손글씨 라인의 낙서풍으로 그린다.
...
- **제목**: 위 약 9~17% 높이 구간. 왼쪽 여백에서 시작해 왼쪽 정렬로 한 줄로 쓴다.
...
- **시선 흐름**: 제목(위) → 본문(중간) → 다이어그램(아래) 순서로 읽히게 세 구역을 확실히 나눈다.
...
`<...>`(문구·다이어그램)만 채우고, 그 외 스타일 문구는 그대로 둔다. 점선 요소가 없으면 그 줄은 지운다.

```text
A portrait explainer "card-news" illustration (4:5, 1080x1350), drawn like a friendly handwritten study
...
HEADER: at the top-left, one line of big bold Korean marker-style handwriting as the title.
...
dashes, generous margins, three clear zones top-to-bottom (title, body text, diagram).

DO NOT: use any color other than black, white, cream and the single royal blue. No characters,
no photos, no texture, no busy background. Do not draw any number badge or circled number.
Do not misspell, translate, or rearrange the Korean text. No watermarks, no logos.
```
...
- 폰트는 저작권 없는 폰트를 사용한다(SIL OFL): 손글씨는 `Gaegu`(개구체), 대체 `Nanum Pen Script`
  ...
  `'Gaegu', 'Nanum Pen Script', sans-serif`로 지정하고 제목은 `font-weight="700"`을 준다.
...
```text
주제: HPA가 CPU 부하에 따라 pod을 자동으로 늘리는 걸 카드뉴스 한 장으로. 문구는 알아서.
```
...
HEADER: at the top-left, one line of big bold Korean marker-style handwriting as the title: "트래픽 급증 대응".
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1350" width="1080" height="1350">
  ...
  <!-- 크림색 종이 배경 -->
  <rect width="1080" height="1350" fill="#F5EFE0"/>

  <!-- 제목: 굵은 손글씨 한 줄 -->
  <text x="84" y="182" text-anchor="start" fill="#1F1D1A"
        font-family="'Gaegu', 'Nanum Pen Script', sans-serif" font-size="92" font-weight="700">트래픽 급증 대응</text>

  <!-- 본문: 2줄, → 로 인과 연결 -->
  <text x="84" y="360" text-anchor="start" fill="#1F1D1A"
        font-family="'Gaegu', 'Nanum Pen Script', sans-serif" font-size="58">CPU 사용률이 임계값을 넘으면</text>
  <text x="84" y="446" text-anchor="start" fill="#1F1D1A"
  ...
  <g stroke="#1F1D1A" stroke-width="7" stroke-linejoin="round" stroke-linecap="round" fill="none">
    <!-- 말풍선: 게이지를 가리킴 -->
    <ellipse cx="230" cy="800" rx="96" ry="66"/>
    <path d="M250 862 L268 906 L296 868"/>

    <!-- CPU 게이지 막대 -->
    <rect x="120" y="940" width="380" height="76" rx="14"/>
    <!-- 채워진 정도 구분선 -->
    <line x1="386" y1="940" x2="386" y2="1016"/>

    <!-- 손그림 화살표 -->
    <path d="M552 978 L648 978 M618 950 L652 978 L618 1006"/>

    <!-- 기존 pod 상자 2개: 실선 -->
    <rect x="690" y="900" width="96" height="150" rx="10"/>
    <rect x="806" y="900" width="96" height="150" rx="10"/>

    <!-- 새로 생길 pod 상자: 점선 -->
    <rect x="922" y="900" width="96" height="150" rx="10" stroke-dasharray="14 12"/>
  </g>

  <!-- 파란 빗금: 게이지의 채워진 부분 (유일한 유채색) -->
  <g stroke="#3B5FD9" stroke-width="6" stroke-linecap="round">
    <line x1="140" y1="1010" x2="192" y2="948"/>
    <line x1="176" y1="1010" x2="228" y2="948"/>
    <line x1="212" y1="1010" x2="264" y2="948"/>
    <line x1="248" y1="1010" x2="300" y2="948"/>
    <line x1="284" y1="1010" x2="336" y2="948"/>
    <line x1="320" y1="1010" x2="372" y2="948"/>
  </g>

  <!-- 파란 반짝 강조: 점선 pod 옆 -->
  <g stroke="#3B5FD9" stroke-width="6" stroke-linecap="round">
    <line x1="1006" y1="868" x2="1022" y2="844"/>
    <line x1="1030" y1="894" x2="1052" y2="882"/>
    <line x1="972" y1="860" x2="976" y2="834"/>
  </g>

  <!-- 말풍선 글씨 -->
  <text x="230" y="822" text-anchor="middle" fill="#1F1D1A"
...
- 크림 배경, 굵은 손글씨 제목, `→` 본문, 낙서 다이어그램의 세 구역 레이아웃을 지켰는가? 번호 배지는
  그리지 않았는가?
