...
#   --toc: Generates a clickable table of contents based on H1~H3 headers.
#   --toc-depth=3: Includes up to H3 in the table of contents.
#
...
do
  ...
  /opt/homebrew/bin/pandoc -s --toc --toc-depth=3 -f markdown+hard_line_breaks+autolink_bare_uris --metadata title="$base" "$f" -o "$output"
