...
```bash
uv run --python 3.12 --with pyyaml python "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/quick_validate.py" plugins/akbun-analysis/skills/akbun-analysiscode
```

Run the existing tests that cover changed behavior and inspect failures semantically; do not replace them with wording-only assertions.
When a bundled script changes, execute its smallest realistic local path and verify its observable output.
