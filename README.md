...
| 고정 스타일의 이미지 생성 프롬프트와 Figma/Canva용 SVG(웹툰·카드뉴스·삽화) | [akbun-draw](#akbun-draw) | `akbun-draw-webtoon-c`, `akbun-draw-cardnews-cream`, `akbun-draw-book-illustration` |
| AWS VPC draw.io 다이어그램, 경계·데이터 흐름 아키텍처 SVG | [akbun-draw-architecture](#akbun-draw-architecture) | `akbun-drawio-aws-vpc`, `akbun-draw-architecture` |
...
아키텍처 그리기 skill 모음. AWS VPC draw.io 다이어그램과 경계·데이터 흐름 아키텍처 SVG를 만든다.

| skill | 설명 |
|---|---|
| [akbun-drawio-aws-vpc](./plugins/akbun-draw-architecture/skills/akbun-drawio-aws-vpc/) | draw.io로 AWS VPC 기초 다이어그램 생성 |
| [akbun-draw-architecture](./plugins/akbun-draw-architecture/skills/akbun-draw-architecture/) | 시스템 구조·데이터 흐름을 점선 경계 박스+역할별 색 컴포넌트+실선 저장/점선 조회 경로의 flat SVG(다크 모드 대응)로 작성 |
...
| [akbun-davinciresolve-workflow](./plugins/akbun-editvideo/skills/akbun-davinciresolve-workflow/) | 색보정 workflow. timeline-chrono → 복제 → 클립별 라벨 노드 준비(`workflow.py nodes`로 점검) → logconvert → exposure → whitebalance → contrast → saturation → sky 순서 실행, 로그 통합. 노드 순서(EXPOSURE→WB→CST→CONTRAST→SAT→SKY)와 실행 순서가 다른 이유 명시 |
...
- `davinciresolve-video-editor`(akbun-editvideo): 편집 순서와 검증·작업 로그를 정하는 오케스트레이터. `davinciresolve-cut-travelflow`, `akbun-davinciresolve-workflow`, `davinciresolve-face-privacy`, `davinciresolve-subtitle-travelnote`, `davinciresolve-audio-delivery`가 그 기본 원칙과 `references/agent-api.md`의 대체 방법 표를 참조한다. 컷이 바뀌면 자막·오디오 skill이 바뀐 구간 목록을 받아 위치를 다시 맞춘다.
- `akbun-davinciresolve-workflow`(akbun-editvideo): 색보정 순서를 정하는 workflow. `akbun-davinciresolve-timeline-chrono`로 시간순 타임라인을 만들고 복제한 뒤 `akbun-davinciresolve-logconvert`, `akbun-davinciresolve-exposure`, `akbun-davinciresolve-whitebalance`, `akbun-davinciresolve-contrast`, `akbun-davinciresolve-saturation`, `akbun-davinciresolve-sky`를 순서대로 실행한다. 측정·세션·라벨 노드 규칙은 exposure skill의 `exposure_scope.py`를 모두가 import한다. 컷은 `davinciresolve-cut-travelflow`가 맡고 색은 이 workflow가 맡는다.
