...
| [davinciresolve-gfx-hyperframes](./plugins/akbun-editvideo/skills/davinciresolve-gfx-hyperframes/) | 챕터 제목·원리·트레이드오프 구조·내 생각·전후 비교표 카드를 HyperFrames(HTML → MP4)로 렌더해 `GFX` 마커 위치 V2에 삽입. 카드 값은 style skill 글자 표, 내용은 비트 시트·전사에서만. `references/hyperframes.md`에 컴포지션·렌더·삽입 절차 |
...
| [davinciresolve-bgm-epidemicsound](./plugins/akbun-editvideo/skills/davinciresolve-bgm-epidemicsound/) | Epidemic Sound 플러그인에서 스타일별 음악 브리프(무드·BPM·보컬 없음)로 검색해 후보 3~5곡 표를 만들고 1곡을 골라 이유와 함께 사용자에게 보고한 뒤 A4 `MUSIC`에 삽입, 곡명·아티스트를 로그에 기록. 다운로드는 사용자 확인 뒤 |
