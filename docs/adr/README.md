...
되돌리기 어렵고, 맥락 없이는 이상해 보이고, 실제 트레이드오프가 있었던 결정만 남긴다. 조건과 형식은 `AGENTS.md`의 결정 기록 규칙을 따른다.

| 번호 | 범위 | 결정 |
|---|---|---|
| [0001](0001-analysis-json-source.md) | akbun-analysiscode | 코드 관계 분석의 단일 원본은 `analysis.json` |
| [0002](0002-drawio-aws-vpc-reference-image-is-not-pixel-target.md) | akbun-drawio-aws-vpc | 참고 이미지는 스타일 출처이고 픽셀 목표가 아님 |
| [0003](0003-kubernets-network-drawio-inputs-are-normalized-to-topology.md) | kubernets-network-drawio | 모든 입력을 하나의 토폴로지로 정규화 |
