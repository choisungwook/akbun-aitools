...
- PR 설명은 한국어로 쓴다. PR body는 diff를 가진 리뷰어가 1분 안에 읽는 브리핑이고, squash 커밋 본문이 된다. 40줄 이내.
- `.github/pull_request_template.md`의 H1 구조(`# 구현`, `# 검증`, `# 어려웠던 점`, `# 리스크`)를 따른다.
- 개조식으로 쓰고 문장은 명사 또는 `-음`, `-함`으로 끝낸다.
- `구현`, `검증`, 기록용 Issue 링크는 항상 남긴다. 내용이 없는 `어려웠던 점`과 `리스크`는 헤더째 삭제한다.
- `구현` 첫 줄은 사용자(또는 이 코드를 쓰는 동료)에게 무엇이 달라지는지 한 문장. 근거는 실제 심볼·경로.
- `검증`은 실제로 돌린 경로마다 "명령 → 결과" 한 줄. 수치는 before → after와 단위. 돌리지 않은 것은 "검증하지 않음"으로 적는다. "통과함"만 쓰지 않는다.
- `리스크`는 누가·무엇이 영향을 받고 왜 안전한지 또는 위험한지, 머지하지 않으면 계속 드는 비용을 1~3문장으로.
- 목표와 의사결정은 기록용 Issue에 두고 PR에는 링크만 남긴다.
- 각 섹션은 요약 한 줄과 필요한 경우 근거 목록 하나까지만 사용한다.
- 쓰지 않는 것: 커밋 SHA 나열, 파일별 체크리스트, 로그·메트릭 표 인용, `CLEAN`·`완벽` 같은 판정어, Summary·Test plan 보일러플레이트. diff와 실제 산출물에서만 쓰고 기억으로 쓰지 않는다.
