...
| PreCompact | `scripts/pre-compact.sh` | compact 직전 캐시 삭제 |
| SessionStart | `scripts/session-start.sh` | /clear 시 캐시 삭제, startup 시 오래된 캐시 정리 |
...
| 이벤트 | 동작 |
|---|---|
| `/clear` | SessionStart(source=clear) → 현재 세션 캐시 삭제 |
| `compact` (컨텍스트 압축) | PreCompact 훅 → 현재 세션 캐시 삭제 |
| `startup` (새 세션) | SessionStart(source=startup) → 7일 이상 된 캐시 파일 정리 |
